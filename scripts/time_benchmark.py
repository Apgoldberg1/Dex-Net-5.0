...
def img_dim_scale_plot(fcgqcnn, fake_fcgqcnn):
    ...
    with torch.no_grad():
        ...
        start_dim, end_dim, step = 40, 90, 5
        ...
        fcgqcnn(torch.zeros(1, 1, 40, 40).to("cuda"))
        start_dim2, end_dim2, step2 = 40, 90, 15

        for img_dim in range(start_dim2, end_dim2, step2):
            start = time.time()
            for _ in range(loops):
                data = torch.rand((batch, 1, img_dim, img_dim)).to(device)
                fake_fcgqcnn(data)
    ...
    plt.title("FC-GQ-CNN vs. Naive Approach Scaling")
    plt.plot(list(range(start_dim2, end_dim2, step2)), time_log2, label = "Naive Approach", color = "orange")
    plt.yscale('log')
    plt.xlabel("image dimension (px)")
    plt.ylabel("inference time per 512 images (log scale, seconds)")
