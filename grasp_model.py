...
class DexNet3(nn.Module):
    def __init__(self):
        super().__init__()
        self.relu = nn.ReLU()
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=7, padding="same")
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=5, padding="same")
        self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding="same")
        self.conv4 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding="same")
        self.lrn = nn.LocalResponseNorm(size=2, alpha=2.0e-05, beta=.75, k=1.0)
